...
--------------------------------------------------
-- Find the skills which role can learn
--------------------------------------------------
function FIGHTSKILL_DATATABLE_Find( fighter )
	local lv = fighter.lv
	local list = {}
	for _, skill in pairs( FIGHTSKILL_DATATABLE ) do
		if not MathUtil_IndexOf( fighter.skills, skill.id ) then
			if skill.conditions then
				for type, value in pairs( skill.conditions ) do
					if FightSkill_MatchCondition( skill, fighter ) then
